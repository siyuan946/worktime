...
2. 依次填写“用户名”“密码”，并在下拉框中选择登录部门：
   - 生产线人员请选择 **“生产部门”**；
   - 工艺、质检等支持岗位请选择 **“工艺部门”**。
3. 若账号在系统中被登记为生产部门，却选择了工艺部门（或反之），系统会提示“该账号所属部门为×××，请按规定选择”，重新选择正确部门后方可登录。
4. 检查键盘大小写是否正确，确认无误后点击 **“登录”**。
5. 登录成功后会自动跳转到主界面，右上角显示当前用户名。
6. 需要离开电脑或交接班时，点击右上角 **“退出登录”**，以免账号被他人使用。

> **提示**：如果密码或部门选择错误，系统会在登录框下方给出提示信息；连续多次失败，请联系管理员核对账号或重置密码。
...
- **扫码录入**：使用条形码快速查询或补录记录。（生产部门隐藏）
- **人员管理**：维护员工工号、姓名、班组等信息。（生产部门隐藏）
- **工序代码**：维护工序代号与名称的对应关系。（生产部门隐藏）
- **操作记录**：查看谁在什么时候做过哪些操作。

当以生产部门身份登录时，为避免误操作，系统会自动隐藏扫码录入、人员管理和工序代码三个入口；即便手动输入地址也会被重定向回 Excel 上传页面。工艺部门账号可正常访问全部功能。
