...
export default {
  ...
  methods: {
    ...
    async remove() {
      ...
      this.loading = true
      try {
        await axios.delete(`/api/api/files/${this.selectedFileId}`, { headers })
        this.selectedFileId = ''
        this.fileId = null
        this.drawings = []
        this.pageRecords = []
    ...
    async parse() {
      ...
      try {
        ...
        const res = await axios.post('/api/api/workrecords/parse?store=true', data, { headers })
        this.fileId = res.data.fileId
        this.selectedFileId = String(this.fileId)
        this.summary = {
          total: res.data.total || 0,
          codeMissing: res.data.codeMissing || 0,
          hoursMissing: res.data.hoursMissing || 0
        }
        if ((this.summary.codeMissing || 0) > 0 || (this.summary.hoursMissing || 0) > 0) {
          alert(`发现 ${this.summary.codeMissing || 0} 条缺少工序码，${this.summary.hoursMissing || 0} 条缺少工时，请检查。`)
        }
        await this.fetchFiles()
        await this.fetchDrawings()
        this.currentDrawingIndex = 0
    ...
    async deleteRow(index) {
      ...
      if (record.id) {
        ...
        try {
          await axios.delete(`/api/api/workrecords/${record.id}`, { headers })
        } catch (e) {
    ...
    async saveNewRecords(records, headers) {
      ...
      for (let offset = 0; offset < records.length; offset += chunkSize) {
        ...
        await axios.post(`/api/api/workrecords?fileId=${this.fileId}`, chunk, { headers })
      }
    },
    async saveExistingRecords(records, headers) {
      ...
      for (let offset = 0; offset < records.length; offset += chunkSize) {
        ...
        await axios.put('/api/api/workrecords/bulk', chunk, { headers })
      }
    },
    async prevDrawing() {
